...
const cardStrength = ['1','2','3','4','5','6','7','8','9','T','J','Q','K','A'];
const adjustHand = hand => hand.split('').map(card => String.fromCharCode(65 + cardStrength.indexOf(card))).join('');
const getStrength = (hand) => Object.values(hand.split('').reduce((acc, card) => (acc[card] = (acc[card] || 0) + 1, acc), {})).sort((a, b) => b - a).join();
